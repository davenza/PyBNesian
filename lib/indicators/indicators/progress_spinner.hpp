...
public:
  void print_progress() {
    ...
    auto& postfix_text = get_value<details::ProgressBarOption::postfix_text>();
    auto& postfix_max_text_len = get_value<details::ProgressBarOption::max_postfix_text_len>();
    if (postfix_max_text_len == 0)
      postfix_max_text_len = postfix_text.length();
    os << " " << postfix_text
       << std::string(postfix_max_text_len-postfix_text.length(), ' ') << "\r";
    os.flush();
    index_ += 1;
