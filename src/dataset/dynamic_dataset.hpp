...
namespace dataset {
    ...
    template<typename Index>
    inline int size_argument(int, const DynamicVariable<Index>& v) {
        return size_argument(v.variable);
    }
    ...
    DataFrame DynamicDataFrame::loc(const Args&... args) const {
        ...
        int total_size = (size_argument(num_columns() * m_temporal_slices.size(), args) + ...);
