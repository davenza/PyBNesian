...
public:
    ...
    void call(BayesianNetworkBase& model, Operator* new_operator, Score& score, int num_iter) const override {
        PYBIND11_OVERRIDE_PURE(void, Callback, call, &model, new_operator, &score, num_iter);
    }
};
...
void pybindings_algorithms_callbacks(py::module& root) {
    ...
    py::class_<Callback, PyCallback, std::shared_ptr<Callback>>(callbacks, "Callback")
        .def(py::init<>())
        .def("call", &Callback::call);

    py::class_<SaveModel, Callback, std::shared_ptr<SaveModel>>(callbacks, "SaveModel")
        .def(py::init<const std::string&>(), py::arg("folder_name"))
...
void pybindings_algorithms(py::module& root) {
    ...
    py::class_<MeekRules>(algorithms, "MeekRules")
        .def_static("rule1", [](PartiallyDirectedGraph& graph) { return MeekRules::rule1(graph); })
        .def_static("rule1", [](ConditionalPartiallyDirectedGraph& graph) { return MeekRules::rule1(graph); })
        .def_static("rule2", [](PartiallyDirectedGraph& graph) { return MeekRules::rule2(graph); })
        .def_static("rule2", [](ConditionalPartiallyDirectedGraph& graph) { return MeekRules::rule2(graph); })
        .def_static("rule3", [](PartiallyDirectedGraph& graph) { return MeekRules::rule3(graph); })
        .def_static("rule3", [](ConditionalPartiallyDirectedGraph& graph) { return MeekRules::rule3(graph); });
