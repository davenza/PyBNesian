...
public:
    ...
    Combinations(Iter begin, Iter end, int k) : 
                            ...
                            m_num_combinations(
                                std::round(
                                    boost::math::binomial_coefficient<double>(m_elements.size(), 
                                                                              m_k - m_fixed.size())
                                )
                            ) {}

    template<typename Iter, typename IterFixed, util::enable_if_iterator_t<Iter, int> = 0, 
    ...
    template<typename V, typename V2, util::enable_if_vector_of_type_t<V, T, int> = 0,
...
template<typename Iter> Combinations(Iter, Iter, int) -> 
                            Combinations<typename std::iterator_traits<Iter>::value_type>;
template<typename Iter, typename IterFixed> Combinations(Iter, Iter, IterFixed, IterFixed, int) -> 
                            Combinations<typename std::iterator_traits<Iter>::value_type>;
template<typename V> Combinations(V, int) -> 
                            Combinations<typename V::value_type>;
template<typename V, typename V2> Combinations(V, V2, int) -> 
                            Combinations<typename V::value_type>;

template<typename T>
class Combinations2Sets {
public:
    template<typename Iter, typename Iter2, util::enable_if_iterator_t<Iter, int> = 0, 
                                            util::enable_if_iterator_t<Iter2, int> = 0>
    Combinations2Sets(Iter begin_set1, 
                      Iter end_set1, 
                      Iter2 begin_set2, 
                      Iter2 end_set2, 
                      int k) : Combinations2Sets(std::vector(begin_set1, end_set1), 
                                                 std::vector(begin_set2, end_set2),
                                                 k) {
    ...
    template<typename V, typename V2, util::enable_if_vector_of_type_t<V, T, int> = 0,
                                      util::enable_if_vector_of_type_t<V2, T, int> = 0>
    Combinations2Sets(V v1, V2 v2, int k) : m_comb1(),
                                            m_comb2(),
...
template<typename V, typename V2> Combinations2Sets(V, V2, int) -> 
        Combinations2Sets<typename V::value_type>;
...
public:
    ...
    template<typename Iter, util::enable_if_iterator_t<Iter, int> = 0>
    AllSubsets(Iter begin, Iter end, int min_k, int max_k) : m_elements(begin, end),
                                                             m_fixed(),
                                                             m_min_k(min_k),
                                                             m_max_k(max_k),
    ...
    template<typename Iter, typename IterFixed, util::enable_if_iterator_t<Iter, int> = 0,
                                                util::enable_if_iterator_t<IterFixed, int> = 0>
    AllSubsets(Iter begin, Iter end, 
               IterFixed begin_fixed, IterFixed end_fixed, 
               int min_k, int max_k) : m_elements(begin, end),
                                       m_fixed(begin_fixed, end_fixed),
                                       m_min_k(min_k),
                                       m_max_k(max_k),
                                       m_num_combinations(0) {
        static_assert(std::is_same_v<
                    ...
                    >, "The type of fixed and movable elements should be the same.");
        for (int i = min_k; i <= max_k; ++i) {
            m_num_combinations += std::round(boost::math::binomial_coefficient<double>(m_elements.size(), i-m_fixed.size()));
        }
    }

    template<typename V, util::enable_if_vector_of_type_t<V, T, int> = 0>
    AllSubsets(V elements, int min_k, int max_k) : m_elements(elements),
                                                   m_min_k(min_k),
                                                   m_max_k(max_k),
                                                   m_num_combinations(0) {
        for (int i = min_k; i <= max_k; ++i) {
    ...
    template<typename V, typename V2, util::enable_if_vector_of_type_t<V, T, int> = 0,
                                      util::enable_if_vector_of_type_t<V2, T, int> = 0>
    AllSubsets(V elements, V2 fixed, int min_k, int max_k) : m_elements(elements),
                                                             m_fixed(fixed),
                                                             m_min_k(min_k),
                                                             m_max_k(max_k),
                                                             m_num_combinations(0) {
    ...
    public:
        ...
        allsubsets_iterator(const AllSubsets<T>* self, int idx) : m_self(self), 
                                                                  m_idx(idx),
                                                                  m_current_comb(),
                                                                  m_current_iter(),
                                                                  m_current_k(m_self->m_min_k) {
            if (idx == 0) {
                m_current_comb = Combinations(m_self->m_elements.begin(), m_self->m_elements.end(), 
                                              m_self->m_fixed.begin(), m_self->m_fixed.end(),
                                              m_current_k);
        ...
        bool operator==(const allsubsets_iterator& rhs) {
            return (m_idx == rhs.m_idx) && (m_self == rhs.m_self);
        }
...
template<typename Iter> AllSubsets(Iter, Iter, int, int) -> 
                            AllSubsets<typename std::iterator_traits<Iter>::value_type>;
template<typename Iter, typename IterFixed> AllSubsets(Iter, Iter, IterFixed, IterFixed, int, int) -> 
                            AllSubsets<typename std::iterator_traits<Iter>::value_type>;
template<typename V> AllSubsets(V, int, int)->
                                    AllSubsets<typename V::value_type>;
template<typename V, typename V2> AllSubsets(V, V2, int, int)->
                                    AllSubsets<typename V::value_type>;
}
