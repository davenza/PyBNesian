...
def test_cv_disjoint_indices():
    ...
    for (train_df, test_df), (train_indices, test_indices) in zip(cv, cv.indices()):
        ...
        assert np.all(
            test_df.to_pandas().to_numpy() == df.iloc[test_indices, :].to_numpy()
        ), "The CV iterator do not slice the test dataset exactly equal as the CV indices iterator."

        assert (
            np.setdiff1d(nptrain, nptest).shape == nptrain.shape
        ), "The train indices includes test indices"
        assert (
            np.setdiff1d(nptest, nptrain).shape == nptest.shape
        ), "The test indices includes train indices"
        assert np.all(
            np.sort(np.setdiff1d(train_indices, test_indices)) == np.sort(train_indices)
        ), "The train indices includes test indices"
        assert np.all(
            np.sort(np.setdiff1d(test_indices, train_indices)) == np.sort(test_indices)
        ), "The test indices includes train indices"
...
def test_cv_fold():
    ...
    for i, (train_df, test_df) in enumerate(cv):
        ...
        assert train_fold.equals(
            train_df
        ), "Train DataFrame fold() and __iter__ are not equal."
        assert test_fold.equals(
            test_df
        ), "Test DataFrame fold() and __iter__ are not equal."
...
def test_cv_null():
    ...
    for (train_df, test_df), (train_indices, test_indices) in zip(cv, cv.indices()):
        ...
        ), "CV did not remove null instances correctly."
    ...
    for (train_df, test_df), (train_indices, test_indices) in zip(
        cv_include_null, cv_include_null.indices()
    ):
        assert (
            train_df.num_rows + test_df.num_rows
        ...
        assert np.all(
            train_df_mat[~np.isnan(train_df_mat)]
            == train_indices_mat[~np.isnan(train_df_mat)]
        ), "The CV iterator do not slice the train dataset exactly equal as the CV indices iterator."

        assert np.all(
            np.isnan(test_df_mat) == np.isnan(test_indices_mat)
        ), "The null values are wrongly specified in the test DataFrame."
        assert np.all(
            test_df_mat[~np.isnan(test_df_mat)]
            == test_indices_mat[~np.isnan(test_df_mat)]
        ), "The CV iterator do not slice the test dataset exactly equal as the CV indices iterator."
