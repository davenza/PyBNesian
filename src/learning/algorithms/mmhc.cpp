...
namespace learning::algorithms {
    ...
    std::unique_ptr<BayesianNetworkBase> MMHC::estimate(const IndependenceTest& test,
        ...
        if (!util::compatible_score(*bn, score.type())) {
            throw std::invalid_argument("Invalid score " + score.ToString() + 
                                        " for model type " + bn->type().ToString() + ".");
        }
        ...
        for (const auto& blacklist_arc : restrictions.arc_blacklist) {
            hc_blacklist.insert(blacklist_arc);
        }
    ...
    std::unique_ptr<ConditionalBayesianNetworkBase> MMHC::estimate_conditional(const IndependenceTest& test,
        ...
        if (interface_nodes.empty())
            return MMHC::estimate(test, op_set, score, nodes, bn_str, 
