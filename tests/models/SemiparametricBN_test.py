...
def test_fit():
    ...
    for n in spbn.nodes():
        cpd = spbn.cpd(n)
        assert cpd.node_type == NodeType.SemiparametricCPD
        assert cpd.underlying_node_type == NodeType.LinearGaussianCPD
    ...
    cpd_c = spbn.cpd('c')
    assert cpd_c.underlying_node_type != spbn.node_type('c')
