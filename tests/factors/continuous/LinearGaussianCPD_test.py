...
def fit_numpy(_df, variable, evidence):
    ...
    (beta, res, _, _) = np.linalg.lstsq(
        linregress_data, df_na.loc[:, variable], rcond=None
    )
...
def numpy_logpdf(test_df, variable, evidence, beta, variance):
    ...
    result[~isnan_vec] = norm.logpdf(
        test_df.loc[:, variable].to_numpy()[~isnan_vec],
        means[~isnan_vec],
        np.sqrt(variance),
    )
    return result
...
def test_lg_logl():
    ...
    for variable, evidence in [
        ...
        assert np.all(
            np.isclose(
                cpd.logl(test_df),
                numpy_logpdf(test_df, variable, evidence, beta, variance),
            )
        ), (
            "Wrong logl for LinearGaussianCPD("
            + str(variable)
            + " | "
            + str(evidence)
            + ")"
...
def test_lg_logl_null():
    ...
    assert np.all(
        np.isclose(cpd.logl(test_df), cpd2.logl(test_df), equal_nan=True)
    ), "The order of the evidence changes the logl() result."
...
def test_lg_slogl():
    ...
    for variable, evidence in [
        ...
        assert np.all(
            np.isclose(
                cpd.slogl(test_df),
                np.sum(numpy_logpdf(test_df, variable, evidence, beta, variance)),
            )
        ), (
            "Wrong slogl for LinearGaussianCPD("
            + str(variable)
            + " | "
            + str(evidence)
            + ")"
    ...
    ), "The order of the evidence changes the slogl() result."
...
def test_lg_slogl_null():
    ...
    for variable, evidence in [
        ...
        assert np.all(
            np.isclose(
                cpd.slogl(df_null),
                np.nansum(numpy_logpdf(df_null, variable, evidence, beta, variance)),
            )
        ), (
            "Wrong slogl for LinearGaussianCPD("
            + str(variable)
...
def test_lg_cdf():
    ...
    for variable, evidence in [
        ...
        assert np.all(
            np.isclose(
                cpd.cdf(test_df), numpy_cdf(test_df, variable, evidence, beta, variance)
            )
        ), (
            "Wrong cdf for LinearGaussianCPD("
            + str(variable)
            + " | "
            + str(evidence)
            + ")"
...
def test_lg_cdf_null():
    ...
    for variable, evidence in [
        ...
        assert np.all(
            np.isclose(
                cpd.cdf(df_null),
                numpy_cdf(df_null, variable, evidence, beta, variance),
                equal_nan=True,
            )
        ), (
            "Wrong cdf for LinearGaussianCPD("
