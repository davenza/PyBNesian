...
void pybindings_operators(py::module& root) {
    ...
    py::class_<OperatorPool>(operators, "OperatorPool")
        .def(py::init<std::shared_ptr<Score>&, std::vector<std::shared_ptr<OperatorSet>>>())
        .def("cache_scores", [](OperatorPool& self, BayesianNetworkBase& model) {
            self.cache_scores(model);
        })
        .def("find_max", [](OperatorPool& self, BayesianNetworkBase& model) {
            return self.find_max(model);
        })
        .def("find_max", [](OperatorPool& self, BayesianNetworkBase& model, OperatorTabuSet& tabu) {
            return self.find_max(model, tabu);
        })
        .def("update_scores", [](OperatorPool& self, BayesianNetworkBase& model, Operator& op) {
            self.update_scores(model, op);
        });
