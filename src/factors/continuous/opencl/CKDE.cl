...
__kernel void accum_sumexp_mat_cols_double(__global double *mat, 
                                         __private uint mat_rows,
                                         ...
                                         __global double *sums)
{
    ...
    local_block[2*local_id] = exp(mat[IDX(min(2*row_id, mat_rows-1), col_id, mat_rows)]);
    local_block[2*local_id+1] = exp(mat[IDX(min(2*row_id+1, mat_rows-1), col_id, mat_rows)]);
	...
	sums[IDX(group_id, col_id, get_num_groups(0))] = local_block[group_size - 1];
...
__kernel void add_accum_sumexp_mat_cols_double(__global double *mat, 
                                             __private uint mat_rows,
    ...
    uint local_id = get_local_id(0);
    uint group_id = get_group_id(0);
    uint num_groups = get_num_groups(0) + 1;
...
// It is best explained in Blelloch, 1990 "Prefix Sums and Their Applications":
//  https://www.cs.cmu.edu/~guyb/papers/Ble93.pdf
__kernel void accum_sumexp_mat_cols_float(__global float *mat, 
                                         __private uint mat_rows,
...
__kernel void add_accum_sumexp_mat_cols_float(__global float *mat, 
                                             __private uint mat_rows,
                                             __private uint mat_offset,
                                             __private uint size_per_group,
                                             __global float *sums) {
