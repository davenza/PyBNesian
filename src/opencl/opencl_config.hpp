...
{                                                                                                       \
    ...
    if (err_code != CL_SUCCESS) {                                                                       \
        throw std::runtime_error(std::string("Error enqueuing OpenCL kernel. ") +                       \
...
namespace opencl {
    ...
    public:
        ...
        void reduction_cols_offset(const cl::Buffer& input_mat, int input_rows, int input_cols, 
                                    cl::Buffer& output_vec, int output_offset);
        ...
        cl_ulong max_local_memory() {
            return m_max_local_memory_bytes;
        }
    ...
    std::vector<cl::Buffer> OpenCLConfig::create_reduction1d_buffers(int length, const char* kernel_name) {
        ...
        while(current_length > device_max_local_size) {
            auto num_groups = static_cast<int>(std::ceil(static_cast<double>(current_length) / static_cast<double>(device_max_local_size)));
    ...
    void OpenCLConfig::reduction1d(cl::Buffer& input_vec, 
        ...
        RAISE_ENQUEUEKERNEL_ERROR(m_queue.enqueueNDRangeKernel(k_reduction, 
                                                               cl::NullRange,
    ...
    cl::Buffer OpenCLConfig::reduction_cols(const cl::Buffer& input_mat,
        ...
        RAISE_ENQUEUEKERNEL_ERROR(m_queue.enqueueNDRangeKernel(k_reduction,
                                                               cl::NullRange,
                                                               cl::NDRange(global_size, input_cols),
    ...
    void OpenCLConfig::logsumexp_cols_offset(cl::Buffer& input_mat, int input_rows, int input_cols, 
        ...
        RAISE_ENQUEUEKERNEL_ERROR(m_queue.enqueueNDRangeKernel(logsumexp_coeffs,
                                                               cl::NullRange,
                                                               cl::NDRange(input_rows*input_cols),
                                                               cl::NullRange));
        ...
        RAISE_ENQUEUEKERNEL_ERROR(m_queue.enqueueNDRangeKernel(finish_lse,
                                                               cl::NullRange,
                                                               cl::NDRange(input_cols),
                                                               cl::NullRange));
    }
    ...
    cl::Buffer OpenCLConfig::accum_sum_cols(cl::Buffer& mat, int input_rows, int input_cols) {
        ...
        auto device_max_local_wg = std::min(
                                            util::bit_util::previous_power2(
                                                static_cast<int>(free_local_memory / static_cast<double>(2*sizeof(CType)))
                                            ),
                                            static_cast<int>(k_local_size));
        ...
        RAISE_ENQUEUEKERNEL_ERROR(m_queue.enqueueNDRangeKernel(k_accum_sumexp,
                                                               cl::NullRange,
                                                               cl::NDRange(global_wg, input_cols),
                                                               cl::NDRange(local_wg, 1)));
        ...
        if (num_groups > 1) {
            ...
            RAISE_ENQUEUEKERNEL_ERROR(m_queue.enqueueNDRangeKernel(k_add_accum_sumexp,
                                                                   cl::NullRange,
                                                                   cl::NDRange(input_rows - 2*local_wg, input_cols),
                                                                   cl::NullRange));
