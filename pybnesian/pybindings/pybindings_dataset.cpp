...
void pybindings_dataset(py::module& root) {
    ...
    py::class_<CrossValidation>(dataset, "CrossValidation")
        .def(py::init([](DataFrame df, int k, std::optional<unsigned int> seed, bool include_null) {
    ...
    py::class_<DynamicDataFrame>(dataset, "DynamicDataFrame")
        ...
        .def("loc", [](const DynamicDataFrame& self, const DynamicVariable<int>& v) { return self.loc(v); })
        .def("loc", [](const DynamicDataFrame& self, const DynamicVariable<std::string>& v) { return self.loc(v); })
        .def("loc",
             [](const DynamicDataFrame& self, const std::vector<DynamicVariable<int>>& vec) { return self.loc(vec); })
        .def("loc",
