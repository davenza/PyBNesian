...
namespace learning::algorithms {
    ...
    std::optional<std::pair<std::unordered_set<int>, double>> evaluate_multivariate_sepset(const G& g,
                                                                                           const Edge& edge,
                                                                                           Comb& comb,
                                                                                           const IndependenceTest& test,
                                                                                           double alpha) {
    ...
    template<typename G>
    std::optional<std::pair<std::unordered_set<int>, double>> find_multivariate_sepset(const G& g, 
                                                                                       const Edge& edge, 
                                                                                       int sep_size,
                                                                                       const IndependenceTest& test,
                                                                                       double alpha) {
        ...
        if (set1_valid) {
            ...
            std::transform(pa1.begin(), pa1.end(), std::inserter(u1, u1.end()), [&g](int pa) {
                return g.name(pa);
            });
        }
        ...
        if (set2_valid) {
            ...
            std::transform(pa2.begin(), pa2.end(), std::inserter(u2, u2.end()), [&g](int pa) {
                return g.name(pa);
            });
        }
    ...
    PartiallyDirectedGraph PC::estimate(const IndependenceTest& test,
        ...
        learning::algorithms::estimate(skeleton, test, 
                    varc_blacklist, varc_whitelist, vedge_blacklist, vedge_whitelist,
                    alpha, use_sepsets, ambiguous_threshold, allow_bidirected, verbose);
        return skeleton;
    }
    ...
    ConditionalPartiallyDirectedGraph PC::estimate_conditional(const IndependenceTest& test,
        ...
        if (interface_nodes.empty())
            return PC::estimate(test, nodes, varc_blacklist, varc_whitelist, vedge_blacklist, vedge_whitelist, 
                                alpha, use_sepsets, ambiguous_threshold, allow_bidirected, verbose)
                                .conditional_graph();
