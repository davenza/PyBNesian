...
def py_nr_bandwidth(df, variables):
    ...
    k = (
        4
        * d
        * np.sqrt(np.linalg.det(delta))
        / (2 * (delta_inv.dot(delta_inv)).trace() + delta_inv.trace() ** 2)
    )
...
def test_productkde_bandwidth():
    ...
    for variables in [["c", "a", "b"], ["d", "a", "b", "c"]]:
        for instances in [50, 150, 500]:
            ...
            assert np.all(
                np.isclose(cpd.bandwidth, py_nr_bandwidth(df[:instances], variables))
            ...
            assert np.all(
                np.isclose(
                    cpd.bandwidth,
                    py_nr_bandwidth(df[:instances], variables),
                    atol=0.0005,
                )
            ), "Wrong bandwidth computed with normal reference rule."
            ...
            cpd.fit(df.iloc[:instances])
            assert np.all(
                np.isclose(cpd.bandwidth, py_scott_bandwidth(df[:instances], variables))
            ...
            assert np.all(
                np.isclose(
                    cpd.bandwidth,
                    py_scott_bandwidth(df[:instances], variables),
                    atol=0.0005,
                )
            ), "Wrong bandwidth computed with Scott's rule."
    ...
    def _test_productkde_fit_iter(variables, _df, instances):
        ...
        if np.all(_df.dtypes == "float32"):
            assert np.all(
                np.isclose(
                    cpd.bandwidth,
                    py_nr_bandwidth(_df.iloc[:instances], variables),
                    atol=0.0005,
                )
            ), "Wrong bandwidth."
        else:
            assert np.all(
                np.isclose(
                    cpd.bandwidth, py_nr_bandwidth(_df.iloc[:instances], variables)
                )
            ), "Wrong bandwidth."
    ...
    def _test_productkde_fit_null_iter(variables, _df, instances):
        ...
        if np.all(_df.dtypes == "float32"):
            assert np.all(
                np.isclose(
                    cpd.bandwidth,
                    py_nr_bandwidth(_df.iloc[nonnan_indices, :], variables),
                    atol=0.0005,
                )
...
def test_productkde_logl():
    def _test_productkde_logl_iter(variables, _df, _test_df):
        ...
        final_scipy_kde.log_det = (
            2 * np.log(np.diag(final_scipy_kde.cho_cov * np.sqrt(2 * np.pi))).sum()
        )
...
def test_productkde_slogl():
    def _test_productkde_slogl_iter(variables, _df, _test_df):
        ...
        if np.all(_df.dtypes == "float32"):
            assert np.all(
                np.isclose(
                    cpd.slogl(_test_df),
                    final_scipy_kde.logpdf(test_npdata.T).sum(),
            ...
            assert np.all(
                np.isclose(
                    cpd.slogl(_test_df), final_scipy_kde.logpdf(test_npdata.T).sum()
                )
            )
    ...
    cpd2.fit(df_float)
    assert np.all(
        np.isclose(cpd.slogl(test_df_float), cpd2.slogl(test_df_float), atol=0.0005)
    ), "Order of evidence changes slogl() result."
...
def test_productkde_slogl_null():
    ...
    cpd2.fit(df_float)
    assert np.all(
        np.isclose(cpd.slogl(df_null_float), cpd2.slogl(df_null_float), atol=0.0005)
    ), "Order of evidence changes slogl() result."
